...
- `QaaS.PackageMirror.ConfigurationStub/`: an internal schema-generator compatibility shim used only to satisfy build-transitive `QaaS.Configuration` module initializers while keeping the real package out of the mirror and release assets
...
10. creates a fresh GitHub release marked as latest with `qaas-packages.zip` containing the full QaaS bootstrap package set except `QaaS.Configuration`, `QaaS.ElasticBootstrap`, and template packages, `not-qaas-packages.zip` containing the full current external dependency package set, schema download links, and grouped QaaS package versions when release publishing is enabled for that run
