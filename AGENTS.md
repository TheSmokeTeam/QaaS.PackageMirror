...
| `QaaS.PackageMirror.Tools/` | CLI: `sync-restored-packages`, `generate-family-schemas`, `publish-mirror-release` |
...
| `.github/workflows/sync-packages.yml` | the single unified workflow (23KB) |
...
- Tracked repos: Framework, Runner, Mocker, Mocker.CommunicationObjects, Common.{Assertions,Generators,Probes,Processors}. Stable tags preferred for all except QaaS.Runner.
...
Changes here affect release/docs automation for the whole ecosystem — test workflow changes via `workflow_dispatch` with publishing skipped (`--skip-publish` inputs). QaaS harness pipeline for non-trivial changes (rubric ≥7/10); conventional commits.
