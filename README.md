...
The push path is intentionally limited to checkout, .NET setup, build, and tests. It validates changes to the mirror implementation without rebuilding packages, publishing a release, or opening docs PRs. This keeps ordinary CI feedback under two minutes when the hosted runner and NuGet cache are warm enough.
...
12. regenerates the qaas-docs reference docs from the mirrored Runner, Mocker, Framework, Assertions, Generators, Probes, and Processors source tags without pre-deleting existing generated function section pages, validates the generated docs navigation, bundles the stable schema download assets into the docs site, pushes a new docs feature branch, and opens a qaas-docs pull request
